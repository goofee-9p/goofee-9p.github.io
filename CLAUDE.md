...
개인 **글(Contents) + 포트폴리오(Work)** 정적 사이트. GitHub Pages (`main` 브랜치 = 배포).

## ✋ 무엇이든 만들기 전에: DESIGN.md를 먼저 읽으세요

이 저장소에서 **콘텐츠를 추가하거나(글·작업) 디자인/스타일을 바꾸기 전에, 반드시
...
- 에디토리얼 무드 + **담백한 카피**: 세리프(Newsreader + Noto Serif KR), 라벨만 Inter 대문자, 넉넉한 여백, 헤어라인.
- **따뜻한 모노크롬 + 절제된 테라코타 액센트 `#B5512E`** 하나만. 액센트는 아주 소량(번호·링크·짧은 룰)만. **2차 색·그림자 금지.**
- 한글 본문은 세리프로. 한 화면에 액센트는 몇 군데를 넘기지 않기.
- **카피는 사실적·담백하게.** 감성적·미사여구(예: "천천히 길어 올린", "조용히 기록하는") 금지.
  콘텐츠를 "레터"라고 부르지 말 것 — "글 / Contents / No. 00N"을 사용.

## 콘텐츠/구조

- 추가 방법(새 글, Work 항목)은 [`README.md`](README.md) 참고.
- 글(에세이) 본문은 **함부로 고치지 말고 보존**하세요. 레이아웃/구조만 디자인 규칙에 맞춥니다.
